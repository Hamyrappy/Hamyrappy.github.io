...
(async function () {
  ...
  /* --------------------------- methods view ----------------------------- */
  const PIPE = [
    { n: '01 · EXTRACT', t: 'Claims from abstracts', d: 'An LLM decomposes each abstract into atomic contribution claims — before any statistical analysis. Quality is human-validated (κ = 0.821).' },
    ...
    { n: '03 · CLUSTER', t: 'UMAP + HDBSCAN, jointly', d: 'All years are clustered together so a cluster means the same thing in 2020 and 2025. Labels for the top 20 clusters are manually reviewed.' },
  ...
  function renderMethods(m) {
    ...
    const minSampled = Math.min(...m.papersPerYear).toLocaleString();
    ...
    const labelNote = reviewedN
      ? `the ${reviewedN} most prominent carry manually reviewed labels, raw c-TF-IDF descriptors are kept everywhere`
    ...
    document.getElementById('methods-body').innerHTML = `
      ...
      <div class="methods-section">
        ...
        <div class="corpus-table">
          ...
          ${corpusRows}
          <div class="corpus-note">Each year is balanced to ${minSampled} randomly sampled papers (seed 42), so document-frequency shares are comparable across years. ${m.nClaims.toLocaleString()} claims in total; ${noisePct} remain unclustered — one-off contributions. ${m.nClusters} clusters; ${labelNote}.</div>
        </div>
      </div>
      ...
      <div class="methods-section">
        ...
        <p class="methods-p">The pipeline and its validation are described in the accompanying paper (EMNLP 2026 System Demonstrations submission). Extraction quality is human-validated (κ = 0.821); clustering is benchmarked against eight baselines on an external taxonomy and is robust across a 36-cell hyperparameter grid. Code is released under MIT, the claim dataset under CC BY 4.0.</p>

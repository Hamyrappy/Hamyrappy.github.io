...
window.ClusterView = (function () {
  ...
  let currentYear = 'ALL';
  let claimsQuery = '';
  let claimsAiriOnly = false;  // claim list restricted to the extra layer's (AIRI) papers
  ...
  function show(cid) {
    ...
    detail.innerHTML = `
      <div class="detail-wrap">
        ...
        <div class="detail-card detail-card-wide" id="detail-byconf-card">
          <div class="detail-card-title">By source — where this topic lives</div>
        ...
        <div class="claims-card">
          <div class="claims-head">
            ...
            <span id="claims-airi-slot" style="margin-left:auto"></span>
            <input type="text" class="field" id="claims-filter" placeholder="Filter claims…" style="width:220px">
          </div>
    ...
    // One-click "only AIRI papers" filter for the claim list — the question an
    // institute user asks in every topic is "and which of these are ours?".
    const airiSlot = document.getElementById('claims-airi-slot');
    const oSrc = (ACC.state.data.sources || []).find(s => s.role === 'overlay');
    if (airiSlot && oSrc) {
      const b = document.createElement('button');
      b.className = 'link';
      b.title = `Show only claims from ${oSrc.venue} papers in this topic`;
      const sync = () => {
        b.textContent = `◆ ${oSrc.venue} only`;
        b.style.color = claimsAiriOnly ? '#fff' : oSrc.color;
        b.style.background = claimsAiriOnly ? oSrc.color : 'transparent';
        b.style.borderRadius = '4px';
        b.style.padding = '2px 7px';
      };
      sync();
      b.addEventListener('click', () => {
        claimsAiriOnly = !claimsAiriOnly;
        sync();
        claimsShown = CLAIMS_PAGE;
        buildClaims(c);
      });
      airiSlot.appendChild(b);
    }
  ...
  function venueBreakdown(cid) {
    ...
    return srcs.map((s, si) => {
      ...
      return { venue: s.venue, color: s.color, role: s.role, df, papersByYear, nPapers, nClaims, overallDf };
  ...
  function drawByConference(c) {
    ...
    const traces = [];
    rows.forEach(r => {
      // Overlay sources (e.g. AIRI, 65 papers) get a summary chip above but no
      // per-year line: a 2-paper year would spike to 50% and dwarf every venue.
      if (r.role === 'overlay') return;
      ...
      traces.push({
        ...
        hovertemplate: `<b>${ACC.escapeHtml(r.venue)}</b><br>%{x}: %{y:.1f}% of its papers · %{customdata} papers here<extra></extra>`,
      });
  ...
  // Paper indices belonging to the extra layer (AIRI) — for the claim-list filter.
  let _layerPapers = null;
  function layerPapers() {
    if (_layerPapers) return _layerPapers;
    _layerPapers = new Set();
    const d = ACC.state.data, P = d.points, srcs = d.sources || [];
    for (let i = 0; i < P.x.length; i++) {
      if ((srcs[P.source[i]] || {}).role === 'overlay') _layerPapers.add(P.paper[i]);
    }
    return _layerPapers;
  }
  ...
  function clusterClaimIdx(c, year) {
    const pts = ACC.state.data.points;
    // Papers present both in the clustered base and in an overlay source (e.g.
    // AIRI) carry their claims twice in points[]. Keep the first copy — base
    // points precede overlay points — so lists and CSV exports stay dedup'ed.
    const idx = [], seen = new Set();
    for (let i = 0; i < pts.x.length; i++) {
      if (pts.cluster[i] !== c.id || (year !== 'ALL' && pts.year[i] !== year)) continue;
      if (claimsAiriOnly && !layerPapers().has(pts.paper[i])) continue;
      const key = pts.paper[i] + '|' + pts.claim[i];
  ...
  function buildClaims(c) {
    ...
    mk('All years', 'ALL');
    // Tab counts must match the list they open (base + overlay, dedup'ed) —
    // c.claims[] is the base-only trajectory and can disagree with the list.
    const pts = ACC.state.data.points, titles = ACC.state.data.titles;
    const perYear = new Map();
    for (const i of clusterClaimIdx(c, 'ALL')) perYear.set(pts.year[i], (perYear.get(pts.year[i]) || 0) + 1);
    years.forEach(y => mk(`${y} (${perYear.get(y) || 0})`, y));

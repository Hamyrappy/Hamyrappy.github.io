...
window.MapView = (function () {
  ...
  function pinCard(layer, i, px, py) {
    ...
    html += '<div class="pin-actions">' +
      `<a href="${url}" target="_blank">Open in ACL Anthology ↗</a>` +
  ...
  /* ------------------------------ legend -------------------------------- */
  // Topics an extra layer (AIRI) contributes to get a small ◆ in the legend,
  // so its footprint is scannable without opening each cluster.
  let _layerMarks = null;   // cluster id -> {color, n}
  function layerMarks() {
    if (_layerMarks) return _layerMarks;
    _layerMarks = new Map();
    const overlayIds = new Set((ACC.state.data.sources || [])
      .filter(s => s.role === 'overlay').map(s => s.id));
    for (const l of sourceLayers()) {
      if (!overlayIds.has(l.id)) continue;
      for (const c of l.cluster) {
        if (c === -1) continue;
        const m = _layerMarks.get(c) || { color: l.color, venue: l.venue, n: 0 };
        m.n++;
        _layerMarks.set(c, m);
      }
    }
  ...
  function renderLegend() {
    ...
    for (const c of ACC.state.sorted) {
      ...
      item.title = `${c.label} · ${c.size.toLocaleString()} claims · ` +
        `${ACC.fmtPp(c.deltaPp)} since ${ACC.state.data.meta.years[0]}` +
        (mk ? ` · ${mk.venue}: ${mk.n} claim${mk.n === 1 ? '' : 's'} here` : '') +
        ` — click: isolate · ctrl/⌘-click: add · double-click: details`;
      item.innerHTML =
        `<span class="legend-dot" style="background:${ACC.clusterColor(c)}"></span>` +
        `<span class="legend-name">${ACC.escapeHtml(c.label)}</span>` +
        (mk ? `<span style="color:${mk.color};font-size:8.5px;flex:none">◆</span>` : '') +
  ...
  function renderSourcesPop() {
    ...
    for (const s of sources) {
      ...
      lab.innerHTML =
        ...
        `<br><span class="sub">${s.role === 'clustered' ? 'reference corpus' : 'subsample'} · ${(s.nClaims || 0).toLocaleString()} claims</span>`;
      const hl = document.createElement('button');
      hl.type = 'button';
      hl.className = 'src-hl' + (highlightSources.has(s.id) ? ' on' : '');
      hl.title = 'Highlight this source in its own color';
  ...
  // The pink diamonds are the point of an overlay instance — say so on screen,
  // and give a one-click path to the payoff view (Compare, pre-set to the
  // overlay cohort).
  function buildOverlayChip() {
    const host = document.getElementById('overlay-chip');
    if (!host) return;
    const overlays = (ACC.state.data.sources || []).filter(s => s.role === 'overlay');
    if (!overlays.length) return;
    const s = overlays[0];
    host.innerHTML =
      `<span style="color:${s.color};font-weight:600">◆ ${ACC.escapeHtml(s.venue)}</span> ` +
      `· ${(s.nPapers || 0).toLocaleString()} papers · ${(s.nClaims || 0).toLocaleString()} claims — `;
    const go = document.createElement('button');
    go.className = 'link';
    go.textContent = 'Compare →';
    go.addEventListener('click', () => {
      const tab = [...document.querySelectorAll('button.tab')].find(b => b.textContent.trim() === 'Compare');
      if (tab) tab.click();
    });
    host.appendChild(go);
  }
  ...
  function build() {
    ...
    // independently toggleable from the Sources menu. Overlay sources (e.g.
    // AIRI) start highlighted in their own colour so the subsample is visible
    // at a glance — the ◑ toggle in the Sources menu turns it off.
    const sources = ACC.state.data.sources || [];
    visibleSources = new Set(sources.map(s => s.id));
    highlightSources = new Set(sources.filter(s => s.role === 'overlay').map(s => s.id));

...
window.CompareView = (function () {
  ...
  let metric = 'df';            // 'df' = % of papers (default: quantizes in whole papers) · 'mix' = % of clustered claims
  ...
  function buildClaims() {
    ...
    // Overlay copies of claims that already exist in the clustered base (papers
    // present in both layers) get dup=true: venue cohorts keep them (the overlay
    // IS the cohort), but author/keyword cohorts must not count a paper twice.
    const srcs = sources(), P = d.points, abp = d.authorsByPaper || [], src = P.source || [];
    const seen = new Set();
    for (let i = 0; i < P.x.length; i++) {
      const s = srcs[src[i]] || {};
      const venue = s.venue || '?';
      const key = P.paper[i] + '|' + P.claim[i];
      const dup = s.role === 'overlay' && seen.has(key);
      if (s.role !== 'overlay') seen.add(key);
      out.push({ i, y: P.year[i], c: P.cluster[i], pk: src[i] + ':' + P.paper[i], venue, dup,
  ...
  // clipped to years the venue actually covers — 'AACL · 2021–2026' would be a
  // lie when AACL only has 2022 papers).
  function windowLabel(cohort) {
    const ys = allYears();
    const eff = effYears(cohort);
    if (!eff.length) return '—';                                   // empty intersection
    let lo = eff[0], hi = eff[eff.length - 1];
    if (cohort && cohort.type === 'venue') {
      const s = sources().find(x => x.venue === cohort.value);
      if (s && s.years && s.years.length) {
        // Intersect with years the venue actually covers — a biennial venue's
        // span would still overstate (AACL runs 2020/2022 only).
        const yr = s.years.filter(y => y >= lo && y <= hi);
        if (!yr.length) return '—';
        lo = yr[0]; hi = yr[yr.length - 1];
      }
    }
  ...
  function profileAt(cohort, year) {
    ...
    const [lo, hi] = cohortRange(cohort);
    const byC = new Map(), papers = new Set(), clusteredPapers = new Set();
    let nClaims = 0, nClustered = 0;
    if (pred) for (const cl of claims) {
      if (year === 'ALL') { if (cl.y < lo || cl.y > hi) continue; }
      else { if (cl.y !== year || cl.y < lo || cl.y > hi) continue; }
      if (!pred(cl)) continue;
      if (cl.dup && cohort.type !== 'venue') continue;   // don't count dual-membership papers twice
      nClaims++; papers.add(cl.pk);
      // Denominators use CLUSTERED claims/papers only — shares are computed
      // over classified claims, the same convention for every cohort.
      if (cl.c === -1) continue;
      nClustered++; clusteredPapers.add(cl.pk);
      if (metric === 'df') { if (!byC.has(cl.c)) byC.set(cl.c, new Set()); byC.get(cl.c).add(cl.pk); }
      else byC.set(cl.c, (byC.get(cl.c) || 0) + 1);
    }
    const denom = metric === 'df' ? clusteredPapers.size : nClustered;
    const out = new Map(), counts = new Map();
    for (const [c, v] of byC) {
      const num = metric === 'df' ? v.size : v;
      out.set(c, denom ? num / denom * 100 : 0);
      counts.set(c, num);
    }
    return { nPapers: papers.size, nClaims, denom, byCluster: out, counts };
  }
  ...
  function metricWord() { return metric === 'df' ? '% of papers' : '% of clustered claims'; }
  ...
  function drawSummary() {
    ...
    el.innerHTML = chip(A, 'A') + chip(B, 'B') +
      `<span class="cmp-note">${metricWord()} · year filter: ${gl}</span>`;
  }
  ...
  function drawProfiles(cohorts) {
    ...
    if (!cohorts.length) { host.innerHTML = '<div class="cmp-empty">Pick a selection to see its topic profile.</div>'; return; }
  ...
  function drawRankedBars(div, cohort) {
    ...
    if (!entries.length) { div.innerHTML = '<div class="cmp-empty">No claims in this selection.</div>'; return; }
    const p = ACC.pal();
    ...
    Plotly.react(div, [{
      ...
      customdata: rows.map(([c]) => [c, prof.counts.get(c) || 0]),
      hovertemplate: `%{y}<br>%{x:.1f}% (%{customdata[1]} of ${prof.denom} ${metric === 'df' ? 'papers' : 'clustered claims'})<br><span style="font-size:10px">click to browse its claims →</span><extra></extra>`,
    }], ACC.plBase({
    ...
    div.on('plotly_click', ev => {
      ...
      openClaims(Array.isArray(cd) ? cd[0] : cd, { tag: cohortTag(cohort) });
    });
  }
  ...
  function drawVersus() {
    ...
    const rows = [...ids].map(c => {
      ...
      return { c, a, b, d: a - b, label: clusterLabel(c),
               na: pa.counts.get(c) || 0, nb: pb.counts.get(c) || 0,
               da: pa.denom, db: pb.denom };
  ...
  function drawDiverging(div, rows, cA, cB) {
    ...
    Plotly.react(div, [{
      ...
      customdata: r.map(x => [x.a, x.b, x.na, x.da, x.nb, x.db]),
      text: r.map(x => (x.d >= 0 ? '+' : '') + x.d.toFixed(1)), textposition: 'outside',
      textfont: { size: 9, color: p.fnt }, cliponaxis: false,
      hovertemplate: `<b>%{y}</b><br>${ACC.escapeHtml(cohortLabel(A))}: %{customdata[0]:.1f}% (%{customdata[2]} of %{customdata[3]}) · ` +
        `${ACC.escapeHtml(cohortLabel(B))}: %{customdata[1]:.1f}% (%{customdata[4]} of %{customdata[5]})<br>Δ %{x:+.1f} pp<br><span style="font-size:10px">click to browse its claims →</span><extra></extra>`,
    }], ACC.plBase({
  ...
  function drawDumbbell(div, rows, cA, cB) {
    ...
    const mk = (key, color, tag) => ({ type: 'scatter', mode: 'markers',
      ...
      customdata: r.map(x => [x.c, key === 'a' ? x.na : x.nb, key === 'a' ? x.da : x.db]),
      hovertemplate: `<b>%{y}</b><br>${ACC.escapeHtml(tag)}: %{x:.1f}% (%{customdata[1]} of %{customdata[2]})<br><span style="font-size:10px">click to browse its claims →</span><extra></extra>` });
    Plotly.react(div, [
    ...
    div.on('plotly_click', ev => {
      const cd = ev.points && ev.points[0] && ev.points[0].customdata;
      openClaims(Array.isArray(cd) ? cd[0] : cd);
    });
  }
  ...
  function drawScatter(div, rows, cA, cB) {
    ...
    Plotly.react(div, [
      ...
      { type: 'scatter', mode: 'markers+text', x: rows.map(r => r.a), y: rows.map(r => r.b),
        ...
        customdata: rows.map(r => [r.c, r.na, r.nb]),
        hovertemplate: `<b>%{text}</b><br>${ACC.escapeHtml(cohortLabel(A))}: %{x:.1f}% (%{customdata[1]}) · ${ACC.escapeHtml(cohortLabel(B))}: %{y:.1f}% (%{customdata[2]})<br><span style="font-size:10px">click to browse its claims →</span><extra></extra>` },
    ], ACC.plBase({
    ...
    div.on('plotly_click', ev => { const cd = ev.points[0].customdata; openClaims(Array.isArray(cd) ? cd[0] : cd); });
  }
  ...
  function drawDrift(cohorts) {
    ...
    host.innerHTML = '';
    host.style.display = 'flex'; host.style.flexDirection = 'column'; host.style.gap = '10px';
    if (!cohorts.length) { host.innerHTML = '<div class="cmp-empty">Pick a selection to see how it drifts over the years.</div>'; return; }
    const bar = document.createElement('div'); bar.className = 'panel-modes';
    bar.innerHTML = '<span class="ctl-label">DRIFT AS</span>';
    // Stacked areas only make sense for topic-mix shares (df shares don't sum
    // to 100%, so drawDriftPlot silently falls back to lines) — hide the option.
    const driftOpts = metric === 'mix' ? [['bump', 'Bump chart'], ['stacked', 'Stacked area']]
                                       : [['bump', 'Bump chart']];
    if (metric !== 'mix' && driftMode === 'stacked') driftMode = 'bump';
    bar.appendChild(modeSeg(driftMode, driftOpts,
      v => { driftMode = v; draw(); }));
    host.appendChild(bar);
    const grid = document.createElement('div'); grid.className = 'panel-grid';
    grid.style.gridTemplateColumns = cohorts.length === 2 ? '1fr 1fr' : '1fr';
  ...
  function drawBump(div, cohort) {
    ...
    if (!totals.size) { div.innerHTML = '<div class="cmp-empty">No claims in this selection.</div>'; return; }
    ...
    top.forEach(c => {
      ...
      traces.push({
        ...
        hovertemplate: `<b>${clusterLabel(c)}</b><br>%{x}: rank %{y}<br><span style="font-size:10px">click to browse its claims →</span><extra></extra>`,
      });
  ...
  function drawDriftPlot(div, cohort) {
    ...
    if (firstC < 0) { div.innerHTML = '<div class="cmp-empty">No claims in this selection.</div>'; return; }
  ...
  function claimsFor(cid, cohort, year) {
    ...
    for (const cl of claims) {
      ...
      if (cl.dup && cohort.type !== 'venue') continue;   // same dedupe rule as profileAt
      out.push(cl.i);
    }
  ...
  function renderDrawerList() {
    ...
    count.textContent = drawerQuery
      ...
      : `${total} claim${total === 1 ? '' : 's'} from this selection${drawerYear != null ? ' · ' + drawerYear : ''}`;
    ...
    if (!idx.length) {
      ...
      empty.textContent = total
        ...
        : 'This selection has no claims in this cluster' + (drawerYear != null ? ' in ' + drawerYear : '') + '.';
  ...
  function cohortControl(side) {
    ...
    const types = side === 'A'
      ? [['venue', 'Source'], ['author', 'Author'], ['keyword', 'Keyword']]
      : [['none', '— none —'], ['venue', 'Source'], ['author', 'Author'], ['keyword', 'Keyword']];
    ...
    if (cohort.type === 'venue') {
      ...
      [['alexander-panchenko', 'Panchenko'], ['artem-shelmanov', 'Shelmanov'], ['elena-tutubalina', 'Tutubalina']].forEach(([id, nm]) => {
        if (!idx[id]) return;
      ...
      val.innerHTML = '<span class="cohort-none">no second selection — showing A only</span>';
    }
  ...
  function build() {
    ...
    claims = buildClaims();
    const vs = venues();
    // Prefer an overlaid subsample (e.g. AIRI) as the default cohort A — on
    // instances that carry one, comparing it against a venue is the point.
    // Both cohorts default to the overlay's own year span, so the comparison
    // covers the same window (an institute active since 2021 shouldn't be
    // held against a venue's 2018 profile).
    const oSrc = sources().find(s => s.role === 'overlay');
    const overlay = (oSrc || {}).venue;
    const oYears = (oSrc && oSrc.years && oSrc.years.length)
      ? { y0: oSrc.years[0], y1: oSrc.years[oSrc.years.length - 1] } : {};
    A = Object.assign({ type: 'venue', value: overlay || vs.find(v => v !== 'EMNLP') || vs[0] }, oYears);
    B = Object.assign({ type: 'venue', value: overlay ? (vs.find(v => v !== overlay) || 'EMNLP') : 'EMNLP' }, oYears);
    ...
    document.getElementById('compare-body').innerHTML = `
      <div class="compare-head">
        ...
        <h1 class="compare-h1">Compare thematic profiles</h1>
        <p class="compare-lead">Pick a selection — a source (a venue or ${overlay ? ACC.escapeHtml(overlay) : 'a venue'}), an author, or a keyword set — and optionally narrow it to a range of years. Add a second to compare them directly, or switch to <i>over time</i> to watch one selection drift across the years. Click any topic in a chart to read its actual claims, each linking to the source paper.</p>
        <div class="cohort-quick" id="cmp-presets" style="margin-top:2px"></div>
      </div>
      <div class="cohort-row" id="cohort-row"></div>
      <div class="compare-opts">
        <span class="ctl-label">MEASURE</span>
        <div class="seg" id="cmp-metric">
          <button class="seg-btn active" data-m="df">Paper share</button>
          <button class="seg-btn" data-m="mix">Topic mix</button>
        </div>
        ...
        <select class="select" id="cmp-year" title="Global year filter — intersected with each selection's own year range">${yearOpts}</select>
      </div>
    ...
    // One-click presets for the questions this instance exists to answer.
    const presets = document.getElementById('cmp-presets');
    if (presets && overlay) {
      presets.insertAdjacentHTML('afterbegin', '<span class="ctl-label">QUICK VIEWS</span>');
      const setScope = mode => {
        const overtime = mode === 'overtime';
        document.querySelectorAll('#cmp-view .seg-btn').forEach(x =>
          x.classList.toggle('active', x.dataset.view === (overtime ? 'overtime' : 'snapshot')));
        yearSel.style.display = overtime ? 'none' : '';
        scope = overtime ? 'overtime' : (yearSel.value === 'ALL' ? 'ALL' : +yearSel.value);
      };
      const mk = (label, title, fn) => {
        ...
        b.textContent = label;
        b.title = title;
        b.addEventListener('click', () => { fn(); renderControls(); draw(); });
        presets.appendChild(b);
      };
      const oCohort = () => Object.assign({ type: 'venue', value: overlay }, oYears);
      for (const v of vs.filter(x => x !== overlay).slice(0, 3)) {
        mk(`${overlay} vs ${v}`, `Compare ${overlay} against ${v} over ${oYears.y0}–${oYears.y1}`, () => {
          A = oCohort(); B = Object.assign({ type: 'venue', value: v }, oYears); setScope('snapshot');
        });
      }
      mk(`${overlay} over time`, `${overlay}'s own topic ranks, year by year`, () => {
        A = oCohort(); B = { type: 'none', value: '' }; setScope('overtime');
      });
    }

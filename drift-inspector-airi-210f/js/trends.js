...
window.TrendsView = (function () {
  ...
  function drawTrajectories() {
    ...
    hint.textContent = trajSelection.length
      ? `${trajSelection.length} cluster${trajSelection.length === 1 ? '' : 's'} shown — click bars in the shift chart or sparklines below to add or remove (max ${MAX_TRAJ})`
      : 'click bars in the shift chart or sparklines below to plot trajectories';
